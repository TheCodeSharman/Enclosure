...
frameOffsetV = (enclosureHeight - panelHeight)/2;
...
module interlock() {
    ...
    rotate([0,90,180]) 
        rotate_extrude()
            polygon([[0,0],[0,5],[rb,5],[rb,2.5],[ra,1],[ra,0]]);
}
...
module frameCorner(
    ...
    union() {
       linear_extrude(height=height)
        ...
        translate([thickness+4.5,interlockOffsetX,offsetZ]) 
                interlock($fn=50);
        translate([thickness+4.5,interlockOffsetX,offsetZ+interlockGap])
                interlock($fn=50);
     }
}
